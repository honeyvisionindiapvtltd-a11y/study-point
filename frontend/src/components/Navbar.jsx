  ...
  <div className="flex flex-col">
    ...
    <span className="mt-1 text-[10px] font-medium tracking-wider text-slate-500">
      ...
      <div className="hidden items-center gap-3 md:flex">
        {user ? <><Link to={dashboardPath} className="btn-secondary"><UserCircle size={18}/> Dashboard</Link><button onClick={logout} className="font-semibold text-slate-600">Logout</button></> :
    ...
    {open && <div className="border-t border-slate-100 px-4 pb-5 md:hidden"><div className="flex flex-col gap-4 pt-4">{links.map(([to,label]) => <Link onClick={() => setOpen(false)} key={to} to={to}>{label}</Link>)}{user ? <Link to={dashboardPath}>Dashboard</Link> : <Link to="/login">Login</Link>}</div></div>}
  </header>
}
